...
z.media.MediaElementHandler = class MediaElementHandler {
  ...
  add_media_element(media_stream_info) {
    if (media_stream_info.type !== z.media.MediaType.VIDEO) {
      ...
      this.logger.info(`Created MediaElement of type '${remote_media_element.nodeName.toLowerCase()}' for MediaStream of flow '${media_stream_info.flow_id}'`, remote_media_element);
    }
  }
  ...
  remove_media_element(flow_id) {
    this._get_media_elements(flow_id).forEeach((media_element) => {
      this._destroy_media_element(media_element);
      this.remote_media_elements.remove(media_element);
      this.logger.info(`Deleted MediaElement of type '${media_element.tagName.toLocaleLowerCase()}' for flow '${flow_id}'`);
    });
  }
  ...
  switch_media_element_output(media_device_id) {
    this.remote_media_elements().forEach((media_element) => this._set_media_element_output(media_element, media_device_id));
  }
   ...
   * @private
   * @param {z.media.MediaStreamInfo} media_stream_info - MediaStream information
   * @returns {HTMLAudioElement} HTMLAudioElement that has the stream attached to it
   */
  _create_media_element(media_stream_info) {
    try {
      ...
      return this.logger.error(`Unable to create AudioElement for flow '${media_stream_info.flow_id}'`, error);
    }
  }
  ...
  */
   ...
   * @param {HTMLMediaElement} media_element - HTMLMediaElement to change playback device for
  ...
  _set_media_element_output(media_element, sink_id) {
    if (media_element.setSinkId) {
      ...
      .then(() => {
        return this.logger.info(`Audio output device '${sink_id}' attached to flow '${media_element.dataset['flow_id']}`, media_element);
      })
      .catch((error) => {
        return this.logger.warn(`Failed to attach audio output device '${sink_id}' to flow '${media_element.dataset['flow_id']}': ${error.message}`, error);
      });
    }
  }
};
