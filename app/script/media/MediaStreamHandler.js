...
z.media.MediaStreamHandler = class MediaStreamHandler {
   ...
   * @returns {z.media.MediaStreamHandler} Handler for MediaStreams
   */
  constructor(media_repository) {
    ...
    amplify.subscribe(z.event.WebApp.CALL.MEDIA.ADD_STREAM, this.add_remote_media_stream);
  ...
  release_media_stream() {
    if (this._release_media_stream(this.local_media_stream())) {
      return this.local_media_stream(undefined);
    }
  }
  ...
  replace_media_stream(media_stream_info) {
    let update_promise;
    this.logger.debug(`Received new MediaStream with '${media_stream_info.stream.getTracks().length}' MediaStreamTrack(s)`,
    ...
    if (this.joined_call()) {
      ...
      update_promise = Promise.all((Array.from(this.joined_call().get_flows()).map((flow_et) => flow_et.update_media_stream(media_stream_info))));
    } else {
  ...
  _initiate_media_stream_success(media_stream_info) {
    if (media_stream_info) {
      this.logger.debug(`Received initial MediaStream with '${media_stream_info.stream.getTracks().length}' MediaStreamTrack(s)`,
        {stream: media_stream_info.stream, audio_tracks: media_stream_info.stream.getAudioTracks(), video_tracks: media_stream_info.stream.getVideoTracks()});
      this._set_stream_state(media_stream_info);
      this.local_media_stream(media_stream_info.stream);
    }
  }
  ...
  add_remote_media_stream(media_stream_info) {
    switch (media_stream_info.type) {
      case z.media.MediaType.AUDIO:
        this.remote_media_streams.audio.push(media_stream_info.stream);
        break;
      case z.media.MediaType.AUDIO_VIDEO: case z.media.MediaType.VIDEO:
        this.remote_media_streams.video(media_stream_info.stream);
        break;
    }
